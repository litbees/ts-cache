...
export function CacheResult(params?: CacheParams): MethodDecorator {
  ...
  return (
    ...
    descriptor.value = function (...args: any[]) {
      const cache: Cache = getCache(target, extendedParams);
...
export function CacheKey(): ParameterDecorator {
  return (target: Object, propertyKey: string | symbol, parameterIndex: number) => {
    const indices: number[] =
      Reflect.getMetadata(`${METADATA_KEY_CACHE_KEYS}_${propertyKey.toString()}`, target, propertyKey) || [];
...
export function CachePut(params?: CacheParamsInvoc): MethodDecorator {
  ...
  return (
    ...
    descriptor.value = function (...args: any[]) {
      const cache: Cache = getCache(target, extendedParams);
      const indexValue: number = Reflect.getMetadata(
        `${METADATA_KEY_CACHE_VALUE}_${propertyKey.toString()}`,
        target,
        propertyKey
      );
      const cacheKey: string = getCacheKey(target, propertyKey, args, indexValue);

      if (!extendedParams.afterInvocation && indexValue && indexValue >= 0 && indexValue < args.length) {
...
export function CacheRemove(params?: CacheParamsInvoc): MethodDecorator {
  ...
  return (
    ...
    descriptor.value = function (...args: any[]) {
      ...
      if (!extendedParams.afterInvocation) {
...
function extendParams(cacheParams?: CacheParams): CacheParamsInvoc {
...
function getCache(target: Object, params: CacheParams): Cache {
  ...
  if (!params.cacheName) {
    throw new Error(`No cache name declared`);
  }
...
function getCacheKey(target: Object, propertyKey: string | symbol, args: any[], cacheValueIndex: number = -1): string {
  ...
  const indices: number[] = Reflect.getMetadata(
    `${METADATA_KEY_CACHE_KEYS}_${propertyKey.toString()}`,
    target,
    propertyKey
  );
  if (indices) {
  ...
  if (args.length === 0) {
    throw new Error(
      `Couldn't generate key without params for '${propertyKey.toString()}' method of ${target.constructor.name}`
    );
  }
